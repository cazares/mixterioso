...
def main(argv=None):
    ...
    # ─────────────────────────────────────────────
    # Minimal change: auto-select option "2" (auto main title + custom ending)
    # but let user type the ending text
    # ─────────────────────────────────────────────
    main_title = auto_main_title(slug, meta)
    print()
    print(f"Auto main title: {main_title}")
    try:
        ending = input("Ending (no parentheses) [default: No Bass, 20% Vocals]: ").strip()
    except EOFError:
        ending = ""
    if not ending:
        ending = "No Bass, 20% Vocals"
    title = f"{main_title} ({ending})"

    # Description: skip prompt (empty)
    description = ""
    ...
    # Only confirmation prompt: final one
