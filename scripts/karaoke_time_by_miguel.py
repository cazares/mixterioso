...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--lyrics", required=True,
        help="Path to lyrics .txt (one line per subtitle line)")
    ap.add_argument("--audio", required=True,
        help="Path to input song .mp3 or YouTube URL")

    ap.add_argument("--offset", type=float, default=0,
        help="Global seconds added to each cue start (negative shifts earlier)")
    ap.add_argument("--font-size", type=int, default=140,
        help="ASS subtitle font size")

    ap.add_argument("--csv",
        help="Existing timing CSV to reuse. Must have headers line,start")
    ap.add_argument("--ass",
        help="Existing ASS file to reuse, else regenerated")

    ap.add_argument("--no-prompt", action="store_true",
        help="Run non-interactive where possible (auto-yes, skip tapping)")
    ap.add_argument("--resolution", default="1280x720",
        help="Output WxH, example 1280x720")
    ap.add_argument("--fps", type=int, default=30,
        help="Output FPS")
    ap.add_argument("--bg-color", default="black",
        help="FFmpeg color background")
    ap.add_argument("--line-hold", type=float, default=2.5,
        help="Hold duration for final line (sec)")
    ap.add_argument("--model", default="htdemucs_6s",
        help="Demucs model name")
    ap.add_argument("--device", default=None,
        help="Demucs device (cpu/cuda). Default auto")
    ap.add_argument("--dry-run", action="store_true",
        help="Print plan then exit")

    ap.add_argument("--skip-demucs", action="store_true",
        help="Skip Demucs stem separation and use full mix as-is")
...
def read_timing_csv(csv_path: Path) -> Tuple[List[str], List[float]]:
    """
    Strict reader.
    Only accepts CSV shaped like:
        line,start
        Hey Jude...,0.000
        Take a sad song...,7.020
    """
    lines, starts = [], []
    with csv_path.open("r", encoding="utf-8") as f:
        ...
        # Normalize case but also require these two columns semantically
...
def write_ass(ass_path: Path,
              ...
              default_hold: float):
    """
    Builds ASS with one Dialogue event per line.
    End time = next line start - tiny gap
    Last line = start + default_hold
    """
    n = len(lines)
    ends: List[float] = []
    for i in range(n):
    ...
    with ass_path.open("w", encoding="utf-8") as f:
        ...
        for line, st, en in zip(lines, starts, ends):
            ...
            f.write(
                f"Dialogue: 0,{srt_time(st+offset)},{srt_time(en)},Default,,0,0,0,,{txt}\n"
            )
...
@dataclass
class StemPlan:
    selected: Dict[str, int]   # stems user explicitly set %
    all_levels: Dict[str, int] # every stem -> % 0..100

def interactive_stem_selection(no_prompt: bool) -> StemPlan:
    """
    Ask which stems to alter and by how much.
    If --no-prompt, default 100 for all.
    """
    if no_prompt:
...
def run_demucs_separation(audio_path: Path,
                          ...
                          out_root: Path) -> Path:
    """
    Calls demucs to split into stems if not already present.
    Returns dir containing bass.wav, drums.wav, etc.
    """
    ensure_dir(out_root)
...
def mix_stems_to_file(stem_dir: Path,
    ...
    for stem in STEM_ORDER:
        ...
        if not wav_path.exists():
            die(f"Missing stem file: {wav_path}")
        inputs += ["-i", str(wav_path)]
        level = levels.get(stem, 100)
        if level == 100:
            map_segs.append(f"[{idx}:a]")
        else:
    ...
    # 6 stems expected in htdemucs_6s
...
def render_karaoke_video(audio_path: Path,
                         ass_path: Path,
                         out_mp4: Path,
                         resolution: str,
                         fps: int,
    ...
    """
    2-pass:
    1. generate background video w/ burned ASS
    2. mux w/ audio
    """
...
def main():
    ...
    # If we skip Demucs, we don't require demucs in PATH.
    ensure_bins(require_demucs=not args.skip_demucs)

    # stem selection or bypass
    if args.skip_demucs:
        ...
        any_change = False
        target_audio_for_mix = audio_path
    else:
    ...
    # run Demucs if needed, mix stems
    if not args.skip_demucs:
        ...
        # Build per-stem levels
        levels = dict(stem_plan.all_levels)

        # If user did not actually reduce anything,
        # auto-mute vocals to create "instrumentalish"
        if not any_change:
            levels["vocals"] = 0
    ...
    if csv_path.exists():
        ...
        # fallback manual tapping
        lines = read_text_lines(lyrics_path)
        if not lines:
            die("Lyrics file has no usable lines after trimming.")
        if args.no_prompt:
            die("No CSV provided. --no-prompt prevents interactive tapping.")
    ...
    # build ASS always (overwrite allowed)
    write_ass(
        ass_path,
        w, h,
        args.font_size,
