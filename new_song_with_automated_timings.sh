...
# All-in-one:
#   lyrics (skip if exists)
#   audio  (skip if exists)
#   align  (skip if exists)
#   render (ALWAYS, because you're tweaking layout)
#
# Example:
#   ./new_song_with_automated_timings.sh "John Frusciante" "Central" --font-size 45 --extra-delay 1.0 --hpad-pct 10

if [ $# -lt 2 ]; then
  echo "usage: $0 \"Artist\" \"Title\" [--font-size N] [--hpad-pct N] [--force-lyrics] [--force-audio] [--force-align] [render extra args...]"
  exit 1
fi
