...
#  - intro screen
#  - gap filler symbols
#  - gap-threshold pass-through
#  - REAL per-vocal-pct audio from demucs stems (when available)
#  - ✅ REUSE EXISTING STEMS (no re-demucs if already separated)
...
# try to find demucs (user has multiple envs sometimes)
find_demucs_bin() {
  ...
  if [ -x "$ROOT/demucs_env/bin/demucs" ]; then
    echo "$ROOT/demucs_env/bin/demucs"
    return
  fi
  if python3 -m demucs --help >/dev/null 2>&1; then
    echo "python3 -m demucs"
    return
  fi
  echo ""
}
...
# we check, in order:
#   output/stems/<artist>-<title>/htdemucs_6s/<audio_base>/
#   output/stems/<artist>-<title>/htdemucs/<audio_base>/
#   output/stems/<artist>-<title>/htdemucs/<audio_base>/vocals.wav   (2-stem case)
find_existing_stems_dir() {
  ...
  if [ -d "$stems_export_dir/htdemucs_6s/$audio_base" ]; then
    echo "$stems_export_dir/htdemucs_6s/$audio_base"
    return
  fi
  # 4-stem
  if [ -d "$stems_export_dir/htdemucs/$audio_base" ]; then
    echo "$stems_export_dir/htdemucs/$audio_base"
    return
  fi
  # 2-stem-ish: directory may not exist, but vocals.wav might
...
# --- args -----------------------------------------------------------------
if [ $# -lt 2 ]; then
  err "Usage: $0 \"Artist\" \"Title\" [--font-size N] [--max-chars N] [--offset-video SEC] [--extra-delay SEC] [--hpad-pct N] [--valign ...] [--vocal-pcts \"0 20 100\"] [--gap-threshold 5.0]"
  exit 1
fi
...
# defaults from your latest good run
FONT_SIZE=140
MAX_CHARS=18
OFFSET_VIDEO=-1.0
EXTRA_DELAY=0.0
...
VALIGN=middle
GAP_THRESHOLD=5.0
GAP_DELAY=2.0   # render_from_csv.py supports this now

HAS_VOCAL_PCTS=0
VOCAL_PCTS_STR=""

# either --vocal-pcts OR per-track stems
...
# 2b) TRUE MONO so panned vocals get centered ------------------------------
...
# 6) per-pct: build audio (from stems if we have them) ---------------------
...
for pct in "${RENDER_PCTS[@]}"; do
  ...
  if [ -n "${BEST_STEMS_DIR:-}" ] && [ -d "$BEST_STEMS_DIR" ]; then
    # collect whatever demucs actually produced
    inputs=()
    filters=()
    idx=0
...
EOF
    ...
    # vocals first (name can vary slightly)
